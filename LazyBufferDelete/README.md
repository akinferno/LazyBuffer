...
This removes the Buffer MCU and creates dummy macros for all the calls to the buffer module from within Macro.cfg.
...
2) Edit you _printer.cfg_ file.
    a) Comment out the Sovol _Buffer_Stepper.cfg_ file by adding a _#_ to the beginning of include statement.
...
# Uninstall 
Decided you prefer loud noises and sporadic operation of the buffer? All you have to do is change your printer.cfg back to include the original file. You can either comment mine out, or delete the line completely. Save and reboot and you are back to how you bought it. The LazyBufferDelete.cfg can be deleted if you don't plan to use it again. 

        [include Buffer_Stepper.cfg]
        #[include LazyBufferDelete.cfg]

# CONCERNS
- I have not tested this. There may be some calls in the Macro.cfg that pop up a warning or filament related menu that will only clear if a variable is changed. If you let me know when that is triggered, I can look for that variable and have the "dummy module" clear it the display. It is also possible the variables never switch and everything is fine.
